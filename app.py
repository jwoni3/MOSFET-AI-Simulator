...
with col1:
    ...
    # 배경 및 기본 구조
    fig_struct.add_shape(type="rect", x0=0, y0=0, x1=10, y1=4, fillcolor="#e2e8f0", line=dict(color="#94a3b8", width=1)) 
    fig_struct.add_shape(type="rect", x0=2.5, y0=4, x1=7.5, y1=4.15, fillcolor="#cbd5e1") # SiO2
    fig_struct.add_shape(type="rect", x0=2.5, y0=4.15, x1=7.5, y1=5.0, fillcolor="#334155") # Gate
    
    sd_c = "#f87171" if mos_type == "NMOS" else "#60a5fa"
    fig_struct.add_shape(type="rect", x0=0.5, y0=2.5, x1=2.5, y1=4, fillcolor=sd_c, line=dict(width=0)) # S
    fig_struct.add_shape(type="rect", x0=7.5, y0=2.5, x1=9.5, y1=4, fillcolor=sd_c, line=dict(width=0)) # D
    
    # 채널 시각화 및 Pinch-off 점선
    if op_region != "차단 영역 (Cutoff)":
        ch_c = "#f59e0b"
        if op_region == "선형 영역 (Linear)":
            t_d = 4.0 - 0.15 * (1 - abs_vds / max(v_ov, 0.001))
            fig_struct.add_shape(type="path", path=f"M 2.5 4 L 7.5 4 L 7.5 {t_d} L 2.5 3.85 Z", fillcolor=ch_c, line=dict(width=0))
        else: # Saturation
            p_p = max(3.0, 7.5 - (abs_vds - v_ov) * 0.8)
            fig_struct.add_shape(type="path", path=f"M 2.5 4 L {p_p} 4 L 2.5 3.85 Z", fillcolor=ch_c, line=dict(width=0))
            # [추가 요청] Pinch-off 세로 점선
            fig_struct.add_shape(type="line", x0=p_p, y0=0, x1=p_p, y1=5.5, line=dict(color="#dc2626", width=2, dash="dash"))
            fig_struct.add_annotation(x=p_p, y=5.7, text="Pinch-off", font=dict(color="#dc2626", size=10), showarrow=False)

    # 라벨
    fig_struct.add_annotation(x=1.5, y=3.25, text="S", font=dict(color="white", size=14), showarrow=False)
    fig_struct.add_annotation(x=8.5, y=3.25, text="D", font=dict(color="white", size=14), showarrow=False)
    fig_struct.add_annotation(x=5, y=4.55, text="Gate (G)", font=dict(color="white", size=12), showarrow=False)
    fig_struct.add_annotation(x=5, y=0.5, text="p-Substrate" if mos_type=="NMOS" else "n-Substrate", font=dict(color="#64748b", size=11), showarrow=False)

    fig_struct.update_layout(height=240, margin=dict(l=0, r=0, t=10, b=0), xaxis=dict(visible=False, range=[0, 10]), yaxes=dict(visible=False, range=[0, 6]), plot_bgcolor='rgba(0,0,0,0)')
...
with col3:
    ...
    if ask_ai_btn:
        ...
        st.markdown("""<div style='background-color:#f0fdf4; padding:15px; border-radius:10px; border:1px solid #bbf7d0;'>
            <p style='margin:0; color:#166534; font-size:14px;'>👈 <b>왼쪽 패널</b>에서 전압을 조절하고 버튼을 누르면 AI 해설이 등장합니다!</p>
