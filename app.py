...
with col2:
    ...
    # 전자/정공 배치
    if v_be >= 0.5:
        fig.add_trace(go.Scatter(x=np.random.uniform(0, 10, 40), y=np.random.uniform(ec+0.2, ec+0.8, 40), 
                                 mode='markers', marker=dict(size=6, color='black'), name="전자"))
        fig.add_trace(go.Scatter(x=np.random.uniform(0, 10, 40), y=np.random.uniform(ev-0.8, ev-0.2, 40), 
    ...
    with st.container(border=True):
        ...
        if ask_ai_btn:
            with st.spinner("분석 중..."):
                st.write(f"현재 **{mode}** 상태에 대한 물성 분석 결과가 출력됩니다.")
        else:
